RSpec.describe "Position of the moon" do
  ...
  specify "it calculates the Sun's mean anomaly (M) correctly" do
    expect(calculation.sun_mean_anomaly2.decimal_degrees.round(6)).to eq(97.643514)
  end
  ...
  specify "it calculates the Earth's eccentricity's correction (E) correctly" do
    expect(calculation.correction_eccentricity_of_earth.round(6)).to eq(1.000194)
  end

  specify "it calculates the moon's heliocentric longitude (Σl) correctly" do
    expect(calculation.moon_heliocentric_longitude.round).to eq(-1_127_527)
  end

  specify "it calculates the moon's heliocentric latitude (Σb) correctly" do
    expect(calculation.moon_heliocentric_latitude.round).to eq(-3_229_126)
  end

  specify "it calculates the moon's distance (Σr) correctly" do
    expect(calculation.moon_heliocentric_distance.round).to eq(-16_590_875)
  end

  specify "it calculates the moon's ecliptical longitude (λ) correctly" do
    expect(calculation.moon_ecliptic_longitude.decimal_degrees.round(6)).to eq(133.162655)
  end
  ...
  specify "it calculates the moon's equitorial horizontal parallax (π) correctly" do
    expect(calculation.equatorial_horizontal_parallax.decimal_degrees.round(6)).to eq(0.991990)
  end

  specify "it calculates the earth's nutation in longitude (Δψ)" do
    expect(calculation.nutation_in_longitude.decimal_arcseconds.round(3)).to eq(16.595)
  end

  specify "it calculates the moon's apparent longitude (apparent λ) correctly" do
    expect(calculation.moon_apparent_ecliptic_longitude.decimal_degrees.round(6)).to eq(133.167264) # should be 133.167265
  end

  specify "it calculates the true obliquity of the ecliptic (ε) correctly" do
    expect(calculation.obliquity_of_ecliptic.decimal_degrees.round(6)).to eq(23.440635) # should be 23.440636
  end

  specify "it calculates the moon's apparent right ascension (α) correctly" do
    expect(calculation.moon_right_ascension.decimal_degrees.round(6)).to eq(134.688469) # should be 134.688470 rubocop:disable Layout/LineLength
  end

  specify "it calculates the moon's apparent declination (δ) correctly" do
    expect(calculation.moon_declination.decimal_degrees.round(6)).to eq(13.768367) # should be 13.768368
  end
end
